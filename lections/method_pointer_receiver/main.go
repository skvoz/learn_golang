...
func main() {
	/**
	rule for use simple receive or pointer receiver
	use pointer receiver if you whant to modify receiver
	So the value type or when you want to avoid copying a large amount of data that is automatically copied
	when passing values.
	If neither of these two used value receivers
	**/
	...
	yourCar.changeCar2("VW1", 30000) //not correct method
	(*yourCar).changeCar2("VW2", 30000)
